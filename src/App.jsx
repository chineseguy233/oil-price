...
function App() {
  ...
  const provinceOptions = Object.keys(cityData)
  ...
  useEffect(() => {
    const fetchStations = async () => {
      if (page === 'trip' && tripSubPage === 'stations' && stations.length === 0) {
            ...
            >
              ...
              <div className={`station-discount ${station.discount.includes('降') ? 'has-discount' : ''}`}>
                {station.discount.includes('降') ? (
                  ...
                  <span className="discount-none">{station.discount}</span>
                )}
              </div>
              <div className="station-btns">
                <button 
                  className="station-btn subscribe-btn"
                  onClick={() => alert('订阅成功! 您将收到该加油站的价格变动通知')}
                >
                ...
                <button 
                  ...
                  onClick={() => {
                    const url = `https://apis.map.qq.com/uri/v1/marker?marker=coord:${station.lat},${station.lng};name:${station.name}&referer=myapp`
